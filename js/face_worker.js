...
// --- Main message handler to communicate with Flutter ---
self.onmessage = async (event) => {
    try {
        // Now that the library is pasted above, we don't need to import it.
        // We do need to ensure the models are loaded.
        // `faceapi` is now a global inside this worker scope.
        await faceapi.nets.tinyFaceDetector.loadFromUri('/models');
        await faceapi.nets.faceLandmark68Net.loadFromUri('/models');
        await faceapi.nets.faceRecognitionNet.loadFromUri('/models');

        const { type, payload } = event.data;
        let faceMatcher = null; // Re-create face matcher as needed.

        if (type === "enroll") {
            const detection = await getDescriptorFromBitmap(payload.bitmap);
            if (detection) {
                self.postMessage({ type: "enrollmentSuccess", payload: Array.from(detection.descriptor) });
            } else {
                self.postMessage({ type: "enrollmentFailure" });
            }
        }

        if (type === "prepareMultiple") {
            const allDescriptors = payload.descriptors;
            if (allDescriptors && Object.keys(allDescriptors).length > 0) {
                const labeledFaceDescriptors = Object.keys(allDescriptors).map(
                    name => new faceapi.LabeledFaceDescriptor(name, [new Float32Array(allDescriptors[name])])
                );
                faceMatcher = new faceapi.FaceMatcher(labeledFaceDescriptors, 0.5);
                console.log("Worker: FaceMatcher created and ready.");
                // For this one-time task, we can just return a success message.
                self.postMessage({ type: "prepareSuccess" });
            } else {
                faceMatcher = null;
                console.log("Worker: No enrolled users. FaceMatcher cleared.");
                 self.postMessage({ type: "prepareSuccess" }); // Still a success, just empty
            }
        }

        if (type === "verifyMultiple") {
            // Re-create the matcher with the provided descriptors for this verification task
             const allDescriptors = payload.descriptors;
            if (allDescriptors && Object.keys(allDescriptors).length > 0) {
                 const labeledFaceDescriptors = Object.keys(allDescriptors).map(
                    name => new faceapi.LabeledFaceDescriptor(name, [new Float32Array(allDescriptors[name])])
            ...
            if (!faceMatcher) {
                self.postMessage({ type: "verificationResult", payload: "unknown" });
                return;
            }
            ...
            if (detection) {
                ...
                self.postMessage({ type: "verificationResult", payload: bestMatch.label });
            } else {
                self.postMessage({ type: "verificationResult", payload: "no_face_detected" });
            }
        }

    } catch (error) {
        // **NEW: Report errors back to the main thread!**
        console.error("Error in face_worker.js:", error);
        self.postMessage({ type: "error", payload: error.message });
    }
};

// Helper function to detect a face from an ImageBitmap
async function getDescriptorFromBitmap(imageBitmap) {
    if (!imageBitmap) return null;
    // **FIXED THE TYPO HERE** (was imageabyte)
